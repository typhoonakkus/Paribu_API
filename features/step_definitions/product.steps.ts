...
When('the user requests {int} products from the product list', async function (this: CustomWorld, limit: number) {
  ...
  const response = await ProductService.getProducts(this.requestContext, this.token, limit);
...
When('the user updates the name of the first product', async function (this: CustomWorld) {
  ...
  const response = await ProductService.getProducts(this.requestContext, this.token, 1);
  const body = await response.json();
  const firstProduct = body.products[0];
  updatedProductId = firstProduct.id;

  const updatedName = firstProduct.name + ' - Updated';
  const updateResponse = await ProductService.updateProduct(this.requestContext, this.token, updatedProductId, { name: updatedName });
...
When('the user deletes the updated product', async function (this: CustomWorld) {
  ...
  const deleteResponse = await ProductService.deleteProduct(this.requestContext, this.token, updatedProductId);
